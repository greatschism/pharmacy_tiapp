...
-- Dumping structure for table wg_mscripts.mblox_status_codes
DROP TABLE IF EXISTS `mblox_status_codes`;
CREATE TABLE IF NOT EXISTS `mblox_status_codes` (
  ...
  `message_status_reason` varchar(1000) DEFAULT NULL,
  ...
  PRIMARY KEY (`id`),
  KEY `FK_mblox_status_codes_client_id` (`client_id`),
  CONSTRAINT `FK_mblox_status_codes_client_id` FOREIGN KEY (`client_id`) REFERENCES `clients` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='This table contains the mBlox message status and their inter';
