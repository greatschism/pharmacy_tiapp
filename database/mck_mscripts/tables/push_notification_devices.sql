...
-- Dumping structure for table wg_mscripts.push_notification_devices
...
CREATE TABLE IF NOT EXISTS `push_notification_devices` (
  ...
  `device_type` varchar(40) DEFAULT NULL COMMENT 'Type of apple device',
  `device_id` varchar(200) DEFAULT NULL COMMENT 'Apple device ID',
  ...
  PRIMARY KEY (`id`),
  KEY `FK_push_notification_devices_1` (`customer_id`),
  KEY `FK_push_notification_devices_client_id` (`client_id`),
  CONSTRAINT `FK_push_notification_devices_client_id` FOREIGN KEY (`client_id`) REFERENCES `clients` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `FK_push_notification_devices_customer_id` FOREIGN KEY (`customer_id`) REFERENCES `customers` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='Table to store push notification device ID''s';
