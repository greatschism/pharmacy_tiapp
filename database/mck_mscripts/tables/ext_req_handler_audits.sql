...
-- Dumping structure for table wg_mscripts.ext_req_handler_audits
DROP TABLE IF EXISTS `ext_req_handler_audits`;
CREATE TABLE IF NOT EXISTS `ext_req_handler_audits` (
  ...
  PRIMARY KEY (`id`),
  KEY `FK_ext_req_handler_audits_action_log_id` (`action_log_id`),
  KEY `FK_ext_req_handler_audits_client_id` (`client_id`),
  KEY `FK_ext_req_handler_audits_error_detail_id` (`error_detail_id`),
  CONSTRAINT `FK_ext_req_handler_audits_action_log_id` FOREIGN KEY (`action_log_id`) REFERENCES `action_logs` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `FK_ext_req_handler_audits_client_id` FOREIGN KEY (`client_id`) REFERENCES `clients` (`id`),
  CONSTRAINT `FK_ext_req_handler_audits_error_detail_id` FOREIGN KEY (`error_detail_id`) REFERENCES `error_details` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='Table to log external request handler API calls.';
