...
-- Dumping structure for table wg_mscripts.notifications
...
CREATE TABLE IF NOT EXISTS `notifications` (
  ...
  `send_options` char(1) DEFAULT '0' COMMENT 'Indicates if the advanced frequency send options are enabled',
  PRIMARY KEY (`id`),
  KEY `FK_notifications_client_id` (`client_id`),
  CONSTRAINT `FK_notifications_client_id` FOREIGN KEY (`client_id`) REFERENCES `clients` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='Stores the notification details';
