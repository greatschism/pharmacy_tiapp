...
-- Dumping structure for table wg_mscripts.customer_prescription_histories
DROP TABLE IF EXISTS `customer_prescription_histories`;
CREATE TABLE IF NOT EXISTS `customer_prescription_histories` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'PK of the table',
  `customer_prescription_id` int(10) unsigned DEFAULT NULL COMMENT 'FK to the customer_prescription table',
  ...
  `is_auto_refill` char(1) DEFAULT '0' COMMENT 'Flag to indicates whether opted for AUTO-Refill',
  `is_hidden` char(1) DEFAULT '0',
  `presc_drug_name` varchar(200) DEFAULT NULL COMMENT 'Prescription drug name',
  `sched` varchar(20) DEFAULT NULL COMMENT 'sched field in PDX patient service',
  ...
  `is_active` char(1) DEFAULT NULL COMMENT 'Flag to identify the status of prescription',
  ...
  `is_refillable` varchar(45) DEFAULT NULL,
  `is_doctor_authorisation` varchar(45) DEFAULT NULL,
  `not_refillable_reason_code` tinyint(4) DEFAULT '0',
  PRIMARY KEY (`id`)
